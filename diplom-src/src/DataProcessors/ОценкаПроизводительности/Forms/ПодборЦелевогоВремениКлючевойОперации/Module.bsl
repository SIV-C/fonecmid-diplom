...
Функция ПодборЦелевоеВремяНаСервере(ПараметрыПодбора)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	ЕСТЬNULL(Результат(ЗамерыОперации.КоличествоЗамеров), 0) КАК КоличествоЗамеров
	                      |ИЗ
...
Функция СоответствиеЗамеров(МенеджерВременныхТаблиц)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ
	                      ...
	                      |	Результат(ЗамерыОперации.КоличествоЗамеров) КАК КоличествоЗамеров
	                      |ИЗ
	                      |	ЗамерыОперации КАК ЗамерыОперации
	                      |
	                      |СГРУППИРОВАТЬ ПО
...
Функция ЗначениеAPDEX(МенеджерВременныхТаблиц, ТекущееЦелевоеВремя)
	Запрос = Новый Запрос("ВЫБРАТЬ
	               ...
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК T_4T,
	               |	Результат(ЗамерыОперации.КоличествоЗамеров) КАК N,
	               |	ЕСТЬNULL((Результат(ВЫБОР
	               |			КОГДА ЗамерыОперации.ВремяВыполнения <= &ЦелевоеВремя
	               |				ТОГДА ЗамерыОперации.КоличествоЗамеров
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) + Результат(ВЫБОР
	               |			КОГДА ЗамерыОперации.ВремяВыполнения > &ЦелевоеВремя
	               |					И ЗамерыОперации.ВремяВыполнения <= 4 * &ЦелевоеВремя
	               |				ТОГДА ЗамерыОперации.КоличествоЗамеров
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) / 2) / Результат(ЗамерыОперации.КоличествоЗамеров),0) КАК APDEX
	               |ИЗ
	               |	ЗамерыОперации КАК ЗамерыОперации");
	Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
